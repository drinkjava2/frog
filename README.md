...
真正的模式识别应该是学习阶段和识别阶段分开来，也就是说，在判断阶段，不允许它有辅助味觉信号来绕过识别，测试应该改成是学习阶段（前半段）允许有味觉， 后半段把味觉强迫取消，使它只能根据经验（也就是已经形成的条件反射，或者说惯性）来作出行为。只是很可惜我这么试了试，跑不出结果，也就是说脑的结构生成是失败的，只能期待后续版本来完成这个任务。  
![result23](result23_016d.gif)   
