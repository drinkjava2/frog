...
1.可增加颜色 2.添加存盘选择框，可以在运行期选择存盘 3.添加每维参数分别显示的选择框 4.新增了显示分裂详细过程功能 5.将颜色参数与细胞位置参数产生关联。  
因为参数之间有关联，更容易发生缺色现象，这是因为进化过程中，主要参数（细胞位置）的进化会导致次要参数的分支被误删，然后就很难再补回来了。为了解决这个问题，下一个版本打算改进算法，采用黑白节点的方式，白节点以下为无限分裂并生成细胞，黑节点依然是分裂但不生成实际细胞，除非碰到白节点为止。以前做法是只有根节点是白节点，以后改成在任意位置随机生成黑或白节点。这样变异就包括：黑白节点的生成、消失、上下左右漂移、黑白转换等。  
