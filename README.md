...
2024-11-18 改进单点信号识别，吃食率做到100%  
在目录history\016c1_OneInput目录下，这是个小改进，还是单点信号识别，与上个版本的区别是实现了100%的吃食率，也就是说在食物到达嘴边的同时咬下，没有时间差，实现了预判：  
![result22](result22_016c1.png)   
脑细胞本身信号传递也需要时间的，为了克服这个时间差，所以016c1版本又引入了两个视觉细胞，它可以提前两个虚拟世界的时钟周期看到食物出现和看到食物消失。虽然只是个小改进，但是我不知从哪个小说里看到过“凡是与时间空间相关的能力都不简单”，所以还是单独把这个版本放在一个目录里。   

2024-11-18 失败的两点信号模式识别  
在history\016d目录下，这个更新主要是说明一下目前的进度和问题。  
016d版的目标是让青蛙用阴阳细胞分裂算法自动进化出两点输入的模式识别功能，也就是说食物不再是单个点信号，而是由两个点信号组成，排除掉00这个明显无意义的输入，就有 01,10,11三种图案排列组合，每一轮测试中，只有一种随机图案表示甜食，另外两种表示有毒食物。这个测试给了青蛙两个视觉细胞对应食物的两个点信号、咬细胞、松口细胞，甜味感觉细胞、苦味感觉细胞，试图让青蛙进化出，在几次尝试之后，根据苦味和甜味自动学会识别一串相同图案的食物是否可食，从而正确地遇到一串甜食才咬下，遇到一串毒食或没有食物时就松口（这个逻辑恰好对应与、或、异或模式的识别）。  
016d版的运行结果是，青蛙每遇到一串食物，始终会咬一口，才决定这一串是继续咬下去，还是松口，这不符合我的预期，因为咬一口也会多多少少吃到毒食的，这不是最优解。但是青蛙不知道啊，它只知道味觉这个细胞真好用，有味觉细胞干吗不用？所以当前构造的测试环境(即Judge类)是有问题的，青蛙利用味觉细胞绕过了模识试别这个测试目标。  
那么真正的模式识别应该是什么呢？应该是学习阶段和识别阶段分开来，也就是说，在判断阶段，不允许它有辅助味觉信号来绕过识别，所以，正确的测试应该是学习阶段（前半段）允许有味觉， 后半段把味觉强迫取消，使它只能根据经验（也就是已经形成的条件反射，或者说惯性）来作出行为。只是很可惜我这么试了试，跑不出结果，也就是说脑的结构生成是失败的，只能期待后续版本来完成这个任务。  
![result23](result23_016d.png)   
这个版本要跑出结果有点慢，大约跑几千轮之后，可以看到青蛙的吃食正确率开始明显提高，绿色线条表示作出了正确的咬或松口动作，红色表出示错。但是青蛙每次都会尝一尝食物之后才决定继续吃还是不吃，绕过了模式识别。所以目前这个版本并没有实现两点图像模式识别这个目标。  
目前的程序有点复杂，假想中的脑细胞属性和行为只是胡乱猜想出来的，运行结果也不理想，但是软件的总体架构已经定形了，如果对这个项目感兴趣的朋友也可以在这个架构上改进，看看能不能实现测试目标，在history目录下有关于这个版本的源码导读。  
