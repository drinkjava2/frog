...
这次的构思本来是想模仿生物细胞的分裂,从一个细胞开始，分裂出任意指定的三维形状，并设计了split、goto等基因命令，但是做来做去做不出结果，细胞们就象跳蚤一样不听使唤，最终还是决定放弃，细胞分裂这个算法太难了。细胞分裂的优点不光是更“象”生物，而是可以利用分形原理缩小基因的长度，因为基因相当于一种自带循环和条件判断的计算机语言。  
最终生成三维形状这个目标还是借助简单遗传算法完成，通过细胞在相邻位置随机生成，并在基因里记录每个细胞的坐标的方式来实现，基因命令被删得只剩一个了，就是随机生成细胞。  
用遗传算法来生成任意形状，就好象一个画家在画画，但是画什么根本不知道，只知道听从旁边人的打分，画的好就打高分，画的不好就打低分，这样无限循环下去，最终画的内容会只由打分的人决定。这也是大自然创造生命的方式。  
生成三维形状这个功能是为了以后生成三维脑细胞的触突关联结构做准备，因为神经网络应该空间上是三维的，这样做模式识别会很方便，而早期的随机连线结构损失了空间信息，三维体全息演示的问题则是它是手工排布的，参数多时优化会困难。
