 ...
 *  这个类的showShapeInBrain方法在绘脑图时调用，在脑图里显示脑细胞群的三维形状，用空心圆来表示，这个三维形状就像是一个模子，细胞长在这个模子里的有奖，否则扣分
 */
public class BrainShapeAwardPenalty {
    public static boolean[][][] shape = new boolean[Env.BRAIN_XSIZE][Env.BRAIN_YSIZE][Env.BRAIN_ZSIZE];
    ...
    private static void putPixiel(String str) {
        ...
        int w = c.length;
        int h = c[0].length;
        for (int z = Env.BRAIN_ZSIZE / 2; z < (Env.BRAIN_ZSIZE / 2 + 5); z++) {
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    if (c[x][h - y - 1] > 0 && !Animal.outBrainRange(x, y, z)) {
                        shape[x][y][z] = true;
    ...
    public static void showShapeInBrain(BrainPicture pic) {// 在脑图上显示当前形状
