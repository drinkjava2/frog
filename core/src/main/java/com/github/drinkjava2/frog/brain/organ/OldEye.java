 ...
 * @since 1.0
 */
public class OldEye extends Organ {// 这个Eye是老版的眼睛，只能看到四个方向，但它的调节距离会自动随机调整到一个最佳值，这就是随机试错算法的一个应用
	...
	public void drawOnBrainPicture(Frog f, BrainPicture pic) {// 把自已这个器官在脑图上显示出来
		...
		Zone seeUp = new Zone(x, y + q3Radius, z, qRadius);
		Zone seeDown = new Zone(x, y - q3Radius, z, qRadius);
		Zone seeLeft = new Zone(x - q3Radius, y, z, qRadius);
		Zone seeRight = new Zone(x + q3Radius, y, z, qRadius);
		pic.drawZone(seeUp);
