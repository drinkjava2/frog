...
public class Egg implements Serializable {
	...
	public Egg() {// 无中生有，创建一个蛋，先有蛋，后有鸡
		...
		organs.add(new Eye().setXYRN(100, 400, 100, "Eye").setFixed(false));
	}

	/** Create egg from frog */
	public Egg(Frog frog) { // 青蛙下蛋，每个青蛙的器官会创建自已的副本或变异
	...
	public Egg(Egg x, Egg y) {// 模拟X、Y 染色体合并，两个蛋生成一个新的蛋， X从Y里借一个器官, 通过用进废退原则来筛选
		// x里原来的organ
		...
		if (yOrganSize > 0) {
			Organ borrow = y.organs.get(RandomUtils.nextInt(yOrganSize));
			if (!borrow.isFixed)
				organs.add(borrow);
		}
	}
