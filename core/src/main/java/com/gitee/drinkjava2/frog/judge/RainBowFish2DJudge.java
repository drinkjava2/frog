 ...
 */
public class RainBowFish2DJudge {//NOSONAR
            ...
            for (int y = 0; y < h; y++) {
                for (int x = 0; x < w; x++) {
                    if (c[x][y] > 0) {
                        ...
                        if (!Animal.outBrainRange(p[0], p[1], 0)) {
                            shape[p[0]][p[1]][0] = true;
                            pointList.add(p);
                        }
                    }
                }
        ...
        for (int x = 0; x < Env.BRAIN_CUBE_SIZE; x++)
            for (int y = 0; y < Env.BRAIN_CUBE_SIZE; y++) 
                    for (int i = 1; i <= Cells.GENE_NUMBERS; i++)
                        if ((animal.cells[x][y][0] & (1 << i)) != 0) //如果色彩存
                            if ((animal.cells[x][y][0] & 1) != 0 && x >= (i - 1) * colorWidth && x < (i * colorWidth))
                                animal.awardAA();
                            else
                                animal.penaltyA();
    }
    ...
    public static void show(BrainPicture pic) {// 在脑图上显示当前形状
        for (int[] p : pointList)
            pic.drawCircle(p[0] + 0.5f, p[1] + 0.5f, 0, 1);
    }
