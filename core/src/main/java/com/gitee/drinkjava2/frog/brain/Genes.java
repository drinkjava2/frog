...
public class Genes { //Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    public static void active(Animal a, int step) {
        ...
        //a.pain *= 0.9;
        if (a.no == 1) { //debug
            Logger.info("BEFstep=" + step + ", posHoleSize=" + a.posHoleSize + ", negHoleSize=" + a.negHoleSize + ", eye=" + a.getEng(Genes.EYE_POS) + ", bite=" + a.getEng(Genes.BITE_POS) + ", ate="
                    + a.ateFood + ",wrong=" + a.ateWrong);
        }
            ...
            for (int x = Env.BRAIN_SIZE - 1; x >= 0; x--) {
                ...
                if (a.energys[x][y][z] >= 0.5f) {//如果细胞激活了
                    ...
                    if (hasGene(cell, EYE)) { //如果是视细胞激活
                        a.addEng(BITE_POS, a.energys[x][y][z] * a.posHoleSize);
                        a.addEng(BITE_POS, -a.energys[x][y][z] * a.negHoleSize);
                        a.setEng0(x, y, z);
                    }
        ...
        if (a.no == 1) { //debug
            Logger.info("AFTstep=" + step + ", posHoleSize=" + a.posHoleSize + ", negHoleSize=" + a.negHoleSize + ", eye=" + a.getEng(Genes.EYE_POS) + ", bite=" + a.getEng(Genes.BITE_POS) + ", ate="
                    + a.ateFood + ",wrong=" + a.ateWrong);
        }
    }
