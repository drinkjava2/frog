...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    private static int num(long cell, int n) { //暂未用上，cell以当前基因掩码b开始，连续取n位成为0~2^n之间的整数返回，并把全局静态变量b左移n位
        int result=0;
        long bb=1L;
        for (int i = 0; i < n; i++) {
            if((cell & b) > 0)
    ...
    // 登记细胞间关联(触突树突)
    static {
        register(null, 16 + 16, true, false, 0, 0, -1); //先登记一些基因位，每个基因位的作用（对应各种细胞类型、行为）后面再说
    }
    ...
    public static void active(Animal a, int step) {
        ...
        float posRes = a.consts[start++]; //positive resistance, 正常量电阻，即通常说的权重, 0时为断路，1时为全通，
        float negRes = a.consts[start++]; //negative resistance, 负常量电阻，负线号的常量电阻, 0时为断路，1时为全通，正权重和负权重并不在同一根线上
        ...
        for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版本所有细胞都排成一条线，位于 z轴上 
            ...
            boolean hasNegLines = is(c);

            b = 1; //从头开始，处理与相邻16个细胞之间的正权重能量传递
            if (hasPosLines) {
                float et = e * posRes; //待改正，先暂定所有正权重相同
            ...
            if (hasNegLines) {
                float et = -e * negRes; //待改进，先暂定所有负权重相同
