...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    private static long b = 1; //全局静态变量，以实现iss方法每调用一次b就移位一次的效果

    public static boolean is(long cell) { // 判断cell是否含b这个基因掩码，并左移位b一位
    ...
    // 登记细胞间关联(触突树突)
    static {
        register(null, 16 + 10, true, false, 0, 0, -1); //先登记一些基因位，每个基因位的作用（对应各种细胞类型、行为）后面再说
    }

    private static int[] specialGenes = new int[10];
    ...
    public static void active(Animal a, int step) {
        ...
        float resP = a.consts[start++]; //positive resistance, 正常量电阻，即通常说的权重, 0时为断路，1时为全通，
        float resM = a.consts[start++]; //minus resistance, 负常量电阻，负线号的常量电阻, 0时为断路，1时为全通，正权重和负权重并不在同一根线上
        float cellValve = a.consts[start++]; //细胞激活的阀值，神经元细胞至少能量多少，才会对激活输出细胞
        //float res = a.consts[start++]; //resistance, 常量电阻，即通常说的权重, 0时为断路，1时为全通，

        if (step == 0) {//在首次调用时，统计脑细胞中如果至少有一个基因在细胞里，加点分
            Arrays.fill(specialGenes, 0); 
            for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版
            ...
            for (int i : specialGenes) //统计完成，开始加点分
                a.fat += i;
        }
        ...
        for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版本所有细胞都排成一条线，位于 z轴上 
            ...
            b = 1L << 17; //特殊基因从17开始,基因本身没什么意义，你给它什么假设并且它正好进化出这个掩码位，它就拥有什么行为 

            if (is(c)) {//如果有active基因，此细胞始终激活. iss方法在判断c有无b掩码后，将b左移一位
                a.setEngZ(z, 1);
            }
            ...
            //而且，正权重和负权重每根线条上都是不同的，并不是一个可以共享的常量，存贮这些权重要花很多空间,如果一个细胞有16个正负连线，16*2就要有32个字节来保存权重

            boolean hasPosLines = is(c);
            boolean hasMinLines = is(c);

            b = 1; //从头开始，处理与相邻16个细胞之间的正权重能量传递
            if (hasPosLines) {
                float et = e * resP; //待改正，先暂定所有正权重相同
                for (int i = 0; i < Env.BRAIN_SIZE; i++) { //正权重线条
            ...
            if (hasMinLines) {
                float et = -e * resM; //待改进，先暂定所有负权重相同
                for (int i = 0; i < Env.BRAIN_SIZE; i++) { //能量传到target ceSll
                    if (is(c)) //如果包含某线胞的序号，就传送负能量给这个细胞
                        a.addEng(0, 0, i, et);
                    b = b << 1;
                }
            }
