...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    public static boolean is_(long cell, String name) { // 判断cell是否含b这个基因掩码，并左移位全局静态变量b一位，用下划线命名表示移位，第二个参数赋给基因一个名字以方便调试
    ...
    // 登记细胞间关联(触突树突)
    static {
        register(16 + 20, true, false, 0, 0, NA); //先登记一些基因位，分布在x=0, y=0的z轴上， 每个基因位的作用（对应各种细胞类型、行为）后面再说
    }
    ...
    private static void init(Animal a) {
        {//初始化每个细胞的初始电阻，暂定单个细胞的所有正连线和负连线初始电阻相同
            for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版本所有细胞都排成一条线，位于 z轴上 
                ...
                float pRes = 0.14f * int_(c, 3); //res 在0~1之间，初始值由随机基因决定的常数位置的值决定
    ...
    public static void active(Animal a) {
        ...
        int start = 0; //start是常数数组的起始点，常数与细胞基因无关，不参与空间分布
        ...
        for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版本所有细胞都排成一条线，位于 z轴上 
            ...
            from(16 + 6); //全局变量b从22号基因开始， 因为0~15保存是否与其它细胞存在连线，16~21保存初始化权重常量，这里跳过前22个
            ...
            if (is_(c, "咬")) {//如果是咬细胞，且处于激活态，咬下
                a.bite = true;
            }

            if (is_(c, "停")) {//如果张嘴细胞激活，停止咬
                a.bite = false;
            }
