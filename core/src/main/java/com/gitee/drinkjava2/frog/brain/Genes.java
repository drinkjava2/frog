    ...
    public static void active(Animal a) {
        ...
        for (int y = 0; y <= 2; y++)
            for (int z = 0; z <= 4; z++) {//遍历所有脑细胞  
                ...
                if (step<8 && is(c, 饿) )  //如果细胞有饿基因，在开始时几步会强制激活以模拟饿信号产生， 本版饿细胞下1就是咬细胞
                        e = a.setEngZ(z, 1f);  
                
                if (e>T && is(c, 下1)) //如果有下1基因，代表这个细胞如果激活，会通过连线向下1的细胞传递能量
                        a.addEngZ(z - 1, e);
                
                if (a.inBiting && is(c, 甜) && FoodJudge.isSweetFood())//如果细胞有甜基因，会在咬下且食物是甜食时激活， 本版甜细胞上1就是咬细胞
                       e = a.setEngZ(z, 1f); 
                
                
                if (a.inBiting && is(c, 苦) && FoodJudge.isBitter())//如果细胞有甜基因，会在咬下且食物是甜食时激活， 本版苦细胞上1就是松细胞
                    e = a.setEngZ(z, 1f); 
                
                
                if (e>T && is(c, 上1)) //如果有上1基因，代表这个细胞如果激活，会通过连线向上1的细胞传递能量
                        a.addEngZ(z + 1, e);
                ...
                if (e > T && is(c, 松)) //如果当前细胞有松基因，且能量是激活态，作出松口动作
                        a.stopBite(); 
