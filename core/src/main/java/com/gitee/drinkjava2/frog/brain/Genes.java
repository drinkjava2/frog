...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    static {
        ...
        忆 = register(true, false, 0, 0, -1, "忆"); //先登记一个忆基因  
...
//            assignGene(0, 0, z, 忆);
    ...
    //触突权重调整有三种情况，一是天生的固有的权重，二是反复发生积累产生的权重，三是受奖惩群发信号进行大幅度或反复复盘产生的重复效果累加产生的权重，
    //重复的作用大于强度，重复可以由内部信号奖惩区驱动。
    private static void twoCellAction(Animal a, long c1, long c2, int z1, int z2) {
        ...
        //2.咬动作会激活苦味或甜味感觉细胞, 这个已在上面单个细胞行为中实现
        
        
        //3.任意两个细胞之间相隔的激活都会产生关联, 比如视觉和咬动作，视觉和甜苦味这些信号之间都会有关联
        //TODO
        
        //4.甜/苦细胞的激活会强化最近形成的关联，即影响到原本不相干的视觉细胞和咬动作之间的关联，形成条件反射
        //TODO
