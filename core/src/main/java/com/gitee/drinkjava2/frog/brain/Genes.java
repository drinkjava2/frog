...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
	...
	public static int[] randomDotPos() {
		int i = RandomUtils.nextInt(dots.size());
		Object[] dt = dots.get(i);
		return new int[] { (int) dt[1], (int) dt[2], (int) dt[3] };
	}
	 ...
	 *         掩码做与运算即可
	 */
	public static long register(String name, int maskBits, boolean display, boolean fill, int x_limit, int y_limit,
			int z_limit) {
	...
	public static long EYE1 = registerFill("眼", EYE1_POS); // 视网膜细胞

	public static int[] EYE2_POS = new int[] { 0, 0, 1 };
	public static long EYE2 = registerFill("眼", EYE2_POS); // 视网膜细胞
	...
	public static long BITTER = registerFill("苦", BITTER_POS); // 苦味细胞

	public static int[] BITE_POS = new int[] { 0, 3, 3 };
	public static long BITE = registerFill("咬", BITE_POS); // 咬动作细胞

	public static int[] ACT_POS = new int[] { 0, 1, 3 };
	public static long ACT = registerFill("活", ACT_POS); // 活跃细胞，这个始终激活 `
	...
	public static void active(Animal a, int step) {
		// 这个地方采用连线算法随机增减连线，与基因分裂算法无关，但因为每个细胞都起了个名字，放在这里编程比较方便。从现在开始，连线算法和分裂算法都基于同一个三维数组模型，可以同时使用，但本版只是采用连线算法
		if (RandomUtils.percent(5)) // 这两行随机加减Line
			a.lines.add(new Line(randomDotPos(), randomDotPos()));
		if (RandomUtils.percent(5) && !a.lines.isEmpty())
		...
		// 连线算法很简单，只需要遍历每个连线，调用它的active方法就可以了

		if (1 > 0) // 以下基于分裂算法时要用到，因为分裂算法是以细胞为最小单位，要遍历海量的每个细胞，并针对细胞有多少基因调用它的行为。但本版只是采用连线算法，所以下面代码跳过
			return;
		int x = 0;
