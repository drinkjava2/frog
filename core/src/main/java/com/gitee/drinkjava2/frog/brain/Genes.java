...
public class Genes { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    private static long cell = 0; //cell

    public static boolean is(long geneMask) { // 判断cell是否含某个基因的掩码
    ...
    static {
        ...
        for (int z = 0; z <= 4; z++) { //再把衰基因拷贝一些
            assignGene(0, 1, z, 衰9, 50); //50表示基因出现概率50%
    ...
    public static void active(Animal a) {
        ...
        for (int y = 0; y <= 2; y++)
            for (int z = 0; z <= 4; z++) {//遍历所有脑细胞  
                //======================先针对单个细胞的行为=====================
                cell = a.cells[x][y][z];
                if (cell == 0) //如果什么基因都没有，则这是个空细胞，跳过
                    continue;

                float e = a.energys[x][y][z];
                if (is(衰9))   //如果有衰基因，细胞的能量按基因衰减率衰退
                    e = a.setEng(x, y, z, 0.9f*e);
                
                if (is(衰8)) 
                    e = a.setEng(x, y, z, 0.8f*e); 
 
                if (step<8 && is(饿) )  //如果细胞有饿基因，在开始时几步会强制激活以模拟饿信号产生， 本版饿细胞下1就是咬细胞
                        e = a.setEngZ(z, 1f);  
                
                if (e>T && is(下1)) //如果有下1基因，代表这个细胞如果激活，会通过连线向下1的细胞传递能量
                        a.addEngZ(z - 1, e);
                
                if (a.inBiting && is(甜) && FoodJudge.isSweetFood())//如果细胞有甜基因，会在咬下且食物是甜食时激活， 本版甜细胞上1就是咬细胞
                       e = a.setEngZ(z, 1f); 
                
                
                if (a.inBiting && is(苦) && FoodJudge.isBitter())//如果细胞有甜基因，会在咬下且食物是甜食时激活， 本版苦细胞上1就是松细胞
                    e = a.setEngZ(z, 1f); 
                
                
                if (e>T && is(上1)) //如果有上1基因，代表这个细胞如果激活，会通过连线向上1的细胞传递能量
                        a.addEngZ(z + 1, e);
             
                
                if (is(咬)) {//如果当前细胞有咬基因，且能量是激活态，作出咬动作
                    if (e > T)
                        a.bite();
                    else
                        a.stopBite();
                ...
                if (e > T && is(松)) //如果当前细胞有松基因，且能量是激活态，作出松口动作
                        a.stopBite(); 

                
                // 视觉细胞 
                if (is(黑0) && FoodJudge.foodBit0) //如果存在点0基因且看到食物的第0位的像素点是黑色
                    a.setEng(x, y, z, 1); //点亮黑0视细胞  

                if (is(白0) && !FoodJudge.foodBit0) //如果存在点0基因且看到食物的第0位的像素点是白色
                    a.setEng(x, y, z, 1); //点亮黑0视细胞  

                if (is(黑1) && FoodJudge.foodBit1) //如果存在点0基因且看到食物的第0位的像素点是黑色
                    a.setEng(x, y, z, 1);//点亮黑0视细胞  

                if (is(白1) && !FoodJudge.foodBit1) //如果存在点0基因且看到食物的第0位的像素点是白色
                    a.setEng(x, y, z, 1); //点亮黑0视细胞  
