...
public class Genesbad { // Genes登记所有的基因， 指定每个基因允许分布的空间范围。注意登录完后还要并针对每个基因在active方法里写出它对应的细胞行为
    ...
    public static void active(Animal a, int step) {
        ...
        for (int z = 0; z < Env.BRAIN_SIZE; z++) {//本版本所有细胞都排成一条线，位于 z轴上 
            ...
            b=b<<1;  
            if (is(c, b)) //如果有反相器，能量要反相
                e = 1-e;
 
            b=b<<1;
            if(is(c, b) && a.seeFoodComing) //看到食物正在靠近?
                a.setEngZ(z, 1);
            
            b=b<<1;
            if(is(c, b) && a.seeEmptyComing) //看到食物将要消失?
                a.setEngZ(z, 1);
